...
- `api/` FastAPI + polars + altair (spec generation only, altair reads polars frames directly) + rapidfuzz. Python 3.13 via uv.
  ...
  - `scripts/build_data.py` raw csv/xlsx plus INE census feature services (2024 indicators and 2017 totals, cached as json in `data/raw`) -> `data/processed/*.parquet`. `comunas.parquet` carries `poblacion` (2024) and `poblacion_2017`; `stats.census_for` picks the census closest to the birth year. 2002 and 1992 only exist in REDATAM, not downloadable.
